...
import {updateVideoFrame} from '../../../store/labels/actionCreators';
...
interface IProps {
  ...
  updateVideoFrame: (frameNo: number) => any;
  videoFrameNo: number;
  data: any
}
...
const EditorContainer: React.FC<IProps> = ({
  ...
  const increaseFrameCount = () => {
    if (videoFrameNo < data[activeImageIndex].length-1) {
        updateVideoFrame(videoFrameNo+1);
    }
  };

  const decreaseFrameCount = () => {
    if (videoFrameNo >= 1) {
        updateVideoFrame(videoFrameNo-1);
    }
  };
  ...
  return (
    <div className="EditorContainer">
      ...
      />
      <button onClick={decreaseFrameCount}>{"<"}</button>
      <button onClick={increaseFrameCount}>{">"}</button>
    </div>
  );
};
...
const mapStateToProps = (state: AppState) => ({
  ...
  videoFrameNo: state.labels.videoFrame,
